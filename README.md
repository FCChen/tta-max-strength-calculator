...
這個網頁工具採用 **Backtracking DFS 演算法**，能在毫秒內算出在給定資源下的「最大潛在武力」，並直接列出最省資源的「最佳行動順序」。
...
*   **目標陣型 & 來源:** 選擇你想湊成的陣型。如果該陣型已經在你場上，選擇「已在場上 (0 MA)」。如果要從手上打出，會自動扣除 1 MA；複製別人的會自動扣除 2 MA。
*   **現有兵種數量:** 填入你場上各時代兵種的數量（A, I, II, III 時代）。

### 📝 實戰範例 (TW)

> 你現在在 Age II (二時代)，你想打出手上的**「拿破崙軍隊 (1步 1騎 1砲)」**陣型。
...
*   **操作方式：**
    *   資源輸入：5 MA, 12 礦, 2 人口
    *   陣型選擇：`II: 拿破崙 (1步1騎1砲)`，來源選擇 `從手中打出 (1 MA)`
    *   部隊輸入：步兵A = `2`，騎兵I = `1`
...
This tool uses a highly optimized **Backtracking DFS algorithm** to instantly solve for the maximum possible strength and outputs the exact sequence of actions to achieve it. 
...
*   **Target Tactic & Source:** Select the tactic you want to form. Specify if it is already active (costs 0 MA), in your hand (costs 1 MA), or common (costs 2 MA).
*   **Current Units:** Enter the count of your existing units across different ages (A, I, II, III).

### 📝 Examples (EN)

> You are in Age II and want to play the **"Napoleonic Army (1 Inf, 1 Cav, 1 Art)"** tactic from your hand.
...
*   **How to input:**
    ...
    *   Tactic: `II: Napoleonic (1 Inf 1 Cav 1 Art)`, Source: `From Hand (1 MA)`
